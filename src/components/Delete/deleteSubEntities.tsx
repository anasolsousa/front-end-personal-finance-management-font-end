...
interface DeleteSubEnitiesProps {
  id: string | number;
  token: string | null;
  refetch: () => void;
}

export function DeleteSubEnities({ id, token, refetch }: DeleteSubEnitiesProps) {
  const [isDeleting, setIsDeleting] = useState(false);
  const [alertMessage, setAlertMessage] = useState('');
  const [alertType, setAlertType] = useState<'success' | 'error' | 'warning'>('success');

  const handleDeleteSubEnities = async () => {
    if (!token) {
    ...
    try {
      ...
      if (response.ok) {
        setAlertMessage('SubEntity successfully removed!');
  ...
  return (
    <div>
      <button
        onClick={handleDeleteSubEnities}
        disabled={isDeleting}
